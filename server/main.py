...
# Translate to Hebrew and cache
def get_hebrew_name(product: str) -> str:
    normalized = normalize_product_name(product)
    if normalized in translated_names:
        return translated_names[normalized]

    try:
        result = translate_client.translate(normalized, target_language='he')
        hebrew = result['translatedText']
        translated_names[normalized] = hebrew
        return hebrew
    except Exception as e:
        print(f"Translation error: {e}")
        return normalized

# Pre-translate all existing products on startup
def pretranslate_products():
    all_products = set(list(inventory.keys()) + list(minimum_required.keys()))
    for product in all_products:
        get_hebrew_name(product)

pretranslate_products()
...
async def root():
    ...
    for product, quantity in inventory.items():
        ...
        hebrew_name = get_hebrew_name(product)
        search_url = f"https://www.shufersal.co.il/online/he/search?text={hebrew_name}"
        ...
        if quantity < min_qty:
            alert_html += f"<li style='color: red;'>⚠️ <a href='{search_url}' target='_blank'>{product.capitalize()}</a> is below minimum ({quantity} / {min_qty})</li>"
...
async def add_product(product_name: str = Form(...), quantity: int = Form(...)):
    try:
        ...
        get_hebrew_name(normalized_name)  # Cache the translation
